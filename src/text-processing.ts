...
export function isChromeLine(raw: string): boolean {
	...
	if (ascii.length < clean.length * 0.3) {
		return true;
	}

	if (/shift\+tab|⏵⏵|⏸/.test(clean)) {
		return true;
	}

	if (/^●?\s*high\s*·\s*\/effort/.test(clean)) {
		return true;
	}

	if (/^\s*esc to interr/.test(clean)) {
		return true;
	}

	if (/^\?\s*for shortcuts/.test(clean)) {
		return true;
	}
	...
	// Tool result markers (⎿ Running…)
	if (/^⎿/.test(clean)) {
		return true;
	}

	if (/wormhole-mobile-voice/i.test(clean)) {
		return true;
	}

	// Thinking indicators: ✽ Kneading…, · Cooked for 3s, * Hashing…
	// Pattern: single non-alpha char + space + word with … or "for Xs/Xm"
	// Use pattern match instead of char list — Claude adds new prefixes often
	if (/^\S\s+\S+…/.test(clean)) {
		return true;
	}

	if (/^\S\s+\S+\s+for\s+\d/.test(clean)) {
		return true;
	}
...
export function stripChrome(content: string): string {
	const lines = content.split("\n");
	let stripped = 0;

	while (lines.length > 0 && stripped < 12) {
		if (isChromeLine(lines[lines.length - 1])) {
			lines.pop();
			stripped++;
		} else {
			break;
